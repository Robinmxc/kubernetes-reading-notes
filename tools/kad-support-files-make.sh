...
set -o errexit
set -o pipefail
rm -rf kad-support-files-2.9.1
mkdir  kad-support-files-2.9.1
mkdir  kad-support-files-2.9.1/bin
mkdir  kad-support-files-2.9.1/down 
mkdir  kad-support-files-2.9.1/down/rpms   
...
do
	cp -r /opt/kad/down/${var} ./kad-support-files-2.9.1/down
done
rpmsFile=(5.10.0-60.18.0.50.oe2203.x86_64 4.19.91-26.an8.x86_64 3.10.0-957.el7.x86_64)
for var in ${rpmsFile[@]};
do
	mkdir ./kad-support-files-2.9.1/down/rpms/${var}
	cp -r  /opt/kad/down/rpms/${var} ./kad-support-files-2.9.1/down/rpms/
done
cd    kad-support-files-2.9.1
tar  -zcvf   kad-support-files-2.9.1.tar.gz bin down
cd ..
mv kad-support-files-2.9.1/kad-support-files-2.9.1.tar.gz .
