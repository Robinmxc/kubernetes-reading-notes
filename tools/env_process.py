  ...
  try:
    ...
    if  ".ky10" in output_uname:
      try:
        ...
        print("远程安装python3:服务器IP="+ip+"")
        del_command="sshpass -p "+ ssh_password+  " ssh -p "+ssh_port+" -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null "+ssh_user+"@"+ip+" \""+sudo+" rm -rf /usr/bin/python39;rm -rf /usr/bin/pip3;rm -rf /usr/local/python3;mkdir -p /usr/local/python3 > /dev/null 2>&1;\" "
        ...
        install_command="sshpass -p "+ ssh_password+  " ssh -p "+ssh_port+" -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null "+ssh_user+"@"+ip+" \""+sudo+"tar -xvf /tmp/python3.tar -C /usr/local/python3;cp /usr/local/python3/bin/python3.9 /usr/bin/python39;	cp /usr/local/python3/bin/pip3 /usr/bin/pip3;\" "
        install_command_output = subprocess.check_output(install_command, shell=True)   
        print("远程安装")   
  except Exception :
