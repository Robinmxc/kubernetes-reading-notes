#!/bin/bash
osname=(`uname -r`)
rm -rf /opt/kad/down/fdfs/rpms/${osname}
mkdir -p /opt/kad/down/fdfs/rpms/${osname}
cd /opt/kad/down/fdfs/rpms/${osname}
result=$(echo $osname | grep ".el7.x86_64")
function centos7(){
	echo "centos7 call"
    rpms="autoconf automake cpp gcc gcc-c++  glibc-devel  glibc-headers  keepalived  kernel-headers  keyutils-libs-devel  krb5-devel \
...
rm -rf /opt/kad/down/fdfs/fastdfs.tar.gz
