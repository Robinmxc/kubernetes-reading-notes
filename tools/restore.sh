...
function pg_restore(){
	  ...
	  sshpass -p ${to_password}  scp  -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null ${local_back_dir}/init.sql root@${to_ip}:${remote_back_dir}
 	  ...
 	  sshpass -p ${to_password}  scp  -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null ${local_back_dir}/essdb.sql root@${to_ip}:${remote_back_dir}
	  ...
	  kube_cp_4="kubectl cp ${remote_back_dir}/essdb.sql -n ruijie-sourceid  -c  postgresql  postgresql-0:/"
	  restore_command_del='kubectl exec -n ruijie-sourceid postgresql-0 -- psql -U postgres -d quartz -c  "DROP SCHEMA public CASCADE;CREATE SCHEMA public;"> /dev/null 2>&1'
	  restore_command_del_2='kubectl exec -n ruijie-sourceid postgresql-0 -- psql -U postgres -d xxl_job  -c  "DROP SCHEMA public CASCADE;CREATE SCHEMA public;" > /dev/null 2>&1'
	  restore_command_del_3='kubectl exec -n ruijie-sourceid postgresql-0 -- psql -U postgres -d flowable  -c  "DROP SCHEMA public CASCADE;CREATE SCHEMA public;" > /dev/null 2>&1'
  	  restore_command_del_4='kubectl exec -n ruijie-sourceid postgresql-0 -- psql -U postgres -d essdb  -c  "DROP SCHEMA public CASCADE;CREATE SCHEMA public;" > /dev/null 2>&1'
	  ...
	  restore_command_4="kubectl exec -n ruijie-sourceid postgresql-0 -- psql -U postgres -d essdb -f /essdb.sql > /dev/null 2>&1"
